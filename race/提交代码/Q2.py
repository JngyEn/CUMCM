...
def get_profit_cost_yield(case_number, x1, x2, x3, x4, N):
    ...
    # 拆解后零件回收成本
    c_dismantle_cost = x4 * ((d2 + c3)  + ((1 - x1) * a2 + (1 - x2) * b2 + (1 - x3) * c2)) * c_defective_num
...
optimal_profits = {case: {'profit': float('-inf'), 'combination': None} for case in range(1, 7)}
...
for comb_index, (x1, x2, x3, x4) in enumerate(combinations):
    for case_number in range(1, 7):
        ...
        # 更新最优利润和组合
        if profit > optimal_profits[case_number]['profit']:
            optimal_profits[case_number]['profit'] = profit
            optimal_profits[case_number]['combination'] = (x1, x2, x3, x4)

# 打印最优利润和对应的参数组合
for case_number in range(1, 7):
    print(f"情况 {case_number} 最优利润: {optimal_profits[case_number]['profit']:.2f}, 对应参数组合: {optimal_profits[case_number]['combination']}")
